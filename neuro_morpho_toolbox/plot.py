...
def cell_in_map(neurons_dict, cell_list, metadata, ccf_annotation,
    ...
    if color.lower() == "celltype":
        celltype_color_dict = get_group_colors(metadata=metadata, group_by="CellType", palette="spectral",
                                               return_str=False)
        ...
        if view.lower() == "coronal":  # x   z,y
            ...
            for iter_Region in regions:
                if type(iter_Region) == str:
                    x_range = np.append(x_range,
                                        np.where(ccf_Contour[position, :, :] == nmt.bs.name_to_id(iter_Region))[1])
                    y_range = np.append(y_range,
                                        np.where(ccf_Contour[position, :, :] == nmt.bs.name_to_id(iter_Region))[0])
...
def contourExtract(input_Arr, num_neighbor=6, ROI_list=[]):
    ...
    assert int(num_neighbor) == 6 or int(
        num_neighbor) == 14, "input number of neighbors to be considered should be either 6 or 14"
        ...
        for iter_Y in y_block:
            for iter_Z in z_block:
                ...
                if int(num_neighbor) == 6:
                    result_list.append(
                        Contour_block6(temp_array, x_l, x_u, y_l, y_u, z_l, z_u))  # ,iter_X[0],iter_Y[0],iter_Z[0]))
                else:
                    result_list.append(
                        Contour_block14(temp_array, x_l, x_u, y_l, y_u, z_l, z_u))  # ,iter_X[0],iter_Y[0],iter_Z[0]))
