...
def cell_in_ccf(cell_list, metadata, view="Coronal", margin=0.05, dpi=80, enlarge=1.5, alpha=0.5, ax=None,
                color="classical"):
    assert view in u_views, " ".join((["option 'view_by' should be one of the following: "] + u_views))

    # Background image
    if view == "Coronal":
        nda = (np.max(annotation.array, axis=0) > 0)
        xspace = annotation.space['z']
        yspace = annotation.space['y']
    if view == "Horizontal":
        nda = (np.max(annotation.array, axis=1) > 0)
        xspace = annotation.space['z']
        yspace = annotation.space['x']
    if view == "Sagittal":
        nda = (np.max(annotation.array, axis=2) > 0).transpose()
        xspace = annotation.space['y']
        yspace = annotation.space['x']
    ...
    single_cell_color_dict = single_cell_colors(cell_list)
    for cellname in cell_list:
        Xe, Ye, Ze, Te, Le = swc_to_edges(cellname)
        ...
        if color == "classical":
            ...
            ax.plot(tp.heng[tp.Te == 4], tp.zong[tp.Te == 4], c='magenta', linewidth=linewidth, alpha=alpha)
            cur_color = 'black'
        if color == "single_cell":
            soma_color = rgb_to_list(single_cell_color_dict[cellname])
            ax.plot(tp.heng, tp.zong,
                    c=rgb_to_list(single_cell_color_dict[cellname]),
                    linewidth=linewidth, alpha=alpha)
        if color.lower() == "subtype":
            soma_color = celltypes_col[metadata.loc[cellname, "Subtype"]]
            ax.plot(tp.heng, tp.zong,
                    c=celltypes_col[metadata.loc[cellname, "Subtype"]],
                    linewidth=linewidth, alpha=alpha)
        tp = tp[(tp["Te"] == 1)]
    ...
    return
=======
>>>>>>> f8c892888ace6c8397b9454e70dd441c05f8e6de
